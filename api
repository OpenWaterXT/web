...
export default async function handler(req, res) {
    ...
    try {
      const response = await fetch(url, {
        ...
        headers: {
          ...
          Accept:
            'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet,application/vnd.ms-excel,application/xml,text/xml,text/html,*/*',
  ...
  return res.status(200).json({
    ...
    attempts,
    message:
      'La API funciona, pero no ha encontrado filas reales en Excel/XML/HTML. Revisa cuál es la URL exacta del botón as Excel/as XML.'
  });
}
...
function looksLikeXml(text, contentType, url) {
  return (
    ...
    /xml/i.test(url) ||
    (/^\s*</.test(text) && /<\?xml|<Workbook|<row|<ranking/i.test(text))
  );
}
...
function rowsFromMatrix(matrix, meta) {
  ...
  for (let i = 0; i < Math.min(matrix.length, 30); i++) {
    ...
    if (
      joined.includes('event code') &&
      joined.includes('family name') &&
      joined.includes('given name') &&
...
function parseHtmlTable(html, meta) {
  ...
  for (const table of tables) {
    ...
    for (const row of rows) {
      matrix.push([...row.matchAll(cellRe)].map((m) => stripTags(m[1])));
    }
